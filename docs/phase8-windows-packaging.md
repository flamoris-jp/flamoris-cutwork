...
`mcp/Flamoris.Mcp.Bridge.exe`です。
...
Cutwork editor launcherであり、bridgeは`mcp/Flamoris.Mcp.Bridge.exe`のみであること、ZIPの可読性、required noticesを検査します。
...
MCP serverには`Flamoris.Mcp.Core` 1.1.0、公式`ModelContextProtocol.Core` 2.2.0とproduction dependenciesを含み、
